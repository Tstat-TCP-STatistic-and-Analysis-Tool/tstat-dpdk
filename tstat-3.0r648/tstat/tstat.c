...
struct in_addr crypto_net_list[MAX_CRYPTO_HOSTS];
...
void reopen_logfile(FILE **fp_ref, const char *basename, const char *filename)
{
  ...
  if (zlib_logs)
    ...
    *fp_ref = fopen (logfile, "a");
    //*fp_ref = fopen ("/dev/null", "a");
    //setvbuf ( *fp_ref , NULL , _IOLBF , 1024*8 );
   }
  ...
  switch(block)
   {
     ...
     case TCP_LOG_LAYER7:
       ...
       wfprintf(fp, " c_ssl_SNI:%d", col++);      // server name of SSL client hello message
       wfprintf(fp, " s_ssl_SCN:%d", col++);      // subject name in SSL server certificate

       wfprintf(fp, " c_spdy:%d", col++);        // SSL client SPDY request
       wfprintf(fp, " s_spdy:%d", col++);        // SSL server SPDY support

       wfprintf(fp, " c_last_handshakeT:%d", col++); // SSL client last packet seen before first Application Data time
       wfprintf(fp, " s_last_handshakeT:%d", col++); // SSL server last packet seen before first Application Data time

       wfprintf(fp, " c_appdataT:%d", col++);     // SSL client first Application Data time
       wfprintf(fp, " s_appdataT:%d", col++);     // SSL server first Application Data time

       wfprintf(fp, " c_appdataB:%d", col++);     // SSL client first Application Data rel seqno
       wfprintf(fp, " s_appdataB:%d", col++);     // SSL server first Application Data rel seqno

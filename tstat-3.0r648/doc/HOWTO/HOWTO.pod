...
It is possible, if C<Zlib> is found on the system, to compile Tstat to support the
...
(redirecting stdout/stderr using the C<-z> option). 

Currently Tstat reacts to the C<USR1> and C<INT> signals.

Sending a C<USR1> signal to a running ‘Tstat’ process makes it print the current flow 
statistics to standard error and then resume its operations.

Sending a C<INT> signal to a running ‘Tstat’ process makes it gracefully terminate its 
...
the program pressing C<Ctrl-C> from the control terminal. 

If Tstat is running unsupervised, you should avoid terminating the process using the C<KILL> signal
( C<'signal -KILL'> or C<'signal -9'>), since it will possibly leave the log files in an 
inconsistent state, and you should therefore use either the C<INT> or the C<TERM> signals
 (C<'signal -INT'> or C<'signal -TERM'>).
...
To hide this information, Tstat implements two possible solution. 

The first one is a simple XOR-based obfuscation of the IP address, activated by the 
command line option C<-A mask>. The 32-bit mask is applied B<only> to internal IPv4 
addresses, as defined by either the C<-N> or the C<-M> command line options. If no 
...
list of IP networks to be anonymized using the command line option C<-Y>. 
The networks are indicated in a format similar to the one used for the C<-N> and C<-C>
...
B<original non encrypted> IP addresses.

The encrytion key used by the Crypto-PAn algorithm can be provided either directly on 
the command line, using the C<--keyvalue> option, or from a file, using 
either the C<--keyfile> or the C<--keybase64> options. The C<--keyfile> option
can be used for a plain text ASCII key, while the C<--keybase64> option will treat 
...
32 random bytes read from C</dev/random>, so it will change every time Tstat starts.
Due to the blocking nature of C</dev/random>, it might take some time for Tstat to
...
F<CPanKey_<datetime>>  (where I<datetime> is the current time of the day) that is 
created in the log directory root (the one indicated by the C<-s> command line option).
The generated file is suitable to be later used as the argument of the C<--keybase64> 
option.
...
Obfuscated (C<-A>) addresses are not explicitely indicated in the log files (if they are
internal, and obfuscation was active, they are actually obfuscated). The presence of 
anonymized (C<-Y>) address is explicitely indicated in the log files in the C<c_iscrypto>
and C<s_iscrypto> columns.
The C<-A> and C<-Y> options are not mutually exclusive, so it is possible to obfuscate the
internal addresses and, at the same time, anonymize another, possible overlapping, set of
IP addresses.

[*] L<http://www.cc.gatech.edu/computing/Telecomm/projects/cryptopan/>
...
The configuration file has an C<ini> file syntax:
    ...
    stop_dumping_mask = 0  # bitmask to control the dumping of tcp packets based on flow type
...
Examples:
     ...
     stop_dumping_mask = 262143  # => 11 1111 1111 1111 1111 discard everything except UNKNOWN
...
Copyright (c) 2001-2014 Politecnico di Torino.  All rights reserved.

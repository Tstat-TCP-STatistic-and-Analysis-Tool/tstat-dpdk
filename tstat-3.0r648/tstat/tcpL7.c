...
int ssl_certificate_check(tcp_pair *ptp, void *pdata, int data_length)
{
  ...
  for (i=0;i<data_length-7 && !done ;i++)
   {
     if (base[i]!=0x06 && base[i]!=0x33 && !(base[i]==0x00 && base[i+1]==0x10)) continue;
	...
	if (regexec(&re_ssl_subject,cname, (size_t) 0, NULL, 0)==0)
      ...
      /*  Depending of various factors, the server seems not to use the Next Protocol
          Identification extension (0x3374), but directly the new Application Layer 
          Protocol Negotiation extension (0x0010).
          We look for the 0x0010 pair (and not just for 0x00), and then for the SPDY
          identifier (supposing it's the  first in the list of proposed protocols)
          */ 
     else if ( base[i]==0x00 && base[i+1]==0x10 && 
          base[i+7]==0x73 && base[i+8]==0x70 && base[i+9]==0x64 && base[i+10]==0x79)
      {
        ptp->ssl_server_spdy = 1;
      }
   }

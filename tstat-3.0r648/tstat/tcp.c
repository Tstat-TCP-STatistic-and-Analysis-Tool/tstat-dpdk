...
FindTTP (struct ip *pip, struct tcphdr *ptcp, int *pdir)
{
  ...
  if (elapsed (last_cleaned, current_time) > GARBAGE_FIRE_TIME)
#else
  if (elapsed (last_cleaned, current_time) > GARBAGE_FIRE_TIME)
#endif
    {
...
trace_done_periodic ()
{
 ...
 /* complete the "idle time" calculations using NOW */
  ...
  for (ix = tcp_index; ix < MAX_TCP_PAIRS; ix += MAX_TCP_PAIRS / MAX_TCP_PAIRS_BURST )
    {
      ptp = ttp[ix];
  ...
  tcp_index = (tcp_index + 1) % (MAX_TCP_PAIRS / MAX_TCP_PAIRS_BURST) ;
 ...
 /************ Start cleaning UDP flows *******************/
  ...
  /* Increasing starting index for the next function call */
  udp_index = (udp_index + 1) % (MAX_UDP_PAIRS / MAX_UDP_PAIRS_BURST);
...
/* Record time and data before first Application Data in SSL/TLS connections */
...
void print_video_stats_core(FILE *fp, tcp_pair *ptp_save, tcb *pab, tcb *pba)
{
  ...
  if ((ptp_save->con_type & HTTP_PROTOCOL) && 
     	       ...
     	       ptp_save->http_data==HTTP_YOUTUBE_204 ))
   {
	...
	wfprintf(fp, " %s", id_string);
      }
     else
      {
        /* This is both good for ID16 and ID46 */
        wfprintf(fp, " %s", ptp_save->http_ytid);
      }
   }
